...
template <typename Space, typename View>
void send_recv_slice_mpidt_contig(benchmark::State &, MPI_Comm comm, const Space &space, int rank, const View &v) {
  // Create a slice of the 3D view
  auto sub_view = Kokkos::subview(v, 0, 0, Kokkos::ALL); // contig
...
template <typename Space, typename View>
void send_recv_slice_mpidt_noncontig(benchmark::State &, MPI_Comm comm, const Space &space, int rank, const View &v) {
  // Create a slice of the 3D view
  auto sub_view = Kokkos::subview(v, 0, Kokkos::ALL, 0); // noncontig
...
template <typename Space, typename View>
void send_recv_slice_mpidt_noncontig2D(benchmark::State &, MPI_Comm comm, const Space &space, int rank, const View &v) {
  // Create a slice of the 3D view
  auto sub_view = Kokkos::subview(v, 0, Kokkos::ALL, Kokkos::ALL); //Non-contig. memory
...
void benchmark_3dslice_deepcopy_contig(benchmark::State &state) {
  ...
  state.SetBytesProcessed(sizeof(double) * state.iterations() * a.size() * 2);
}
...
void benchmark_3dslice_mpidt_contig(benchmark::State &state) {
  int rank, size;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size < 2) {
  ...
  while (state.KeepRunning()) {
    do_iteration(state, MPI_COMM_WORLD, send_recv_slice_mpidt_contig<Kokkos::DefaultExecutionSpace, Kokkos::View<double ***>>, space,
                 rank, a);
  }
  state.SetBytesProcessed(sizeof(double) * state.iterations() * a.size() * 2);
}

void benchmark_3dslice_mpidt_noncontig(benchmark::State &state) {
  int rank, size;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size < 2) {
  ...
  while (state.KeepRunning()) {
    do_iteration(state, MPI_COMM_WORLD, send_recv_slice_mpidt_noncontig<Kokkos::DefaultExecutionSpace, Kokkos::View<double ***>>, space,
                 rank, a);
  }
  state.SetBytesProcessed(sizeof(double) * state.iterations() * a.size() * 2);
}

void benchmark_3dslice_mpidt_noncontig2D(benchmark::State &state) {
  int rank, size;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size < 2) {
  ...
  while (state.KeepRunning()) {
    do_iteration(state, MPI_COMM_WORLD, send_recv_slice_mpidt_noncontig2D<Kokkos::DefaultExecutionSpace, Kokkos::View<double ***>>, space,
                 rank, a);
  }
  state.SetBytesProcessed(sizeof(double) * state.iterations() * a.size() * 2);
}
...
BENCHMARK(benchmark_3dslice_mpidt_contig)
    ->UseManualTime()
    ->Unit(benchmark::kMillisecond)
    ->RangeMultiplier(4)
    ->Range(1, 1024);

BENCHMARK(benchmark_3dslice_mpidt_noncontig)
    ->UseManualTime()
    ->Unit(benchmark::kMillisecond)
    ->RangeMultiplier(4)
    ->Range(1, 1024);

BENCHMARK(benchmark_3dslice_mpidt_noncontig2D)
    ->UseManualTime()
    ->Unit(benchmark::kMillisecond)
    ->RangeMultiplier(4)
    ->Range(1, 1024);